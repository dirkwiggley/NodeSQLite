...
class DBTest {
  constructor() {
    ...
    this.init = (res, next) => {
      try {
        let db = this.dbUtils.getDb()
        const create = db.prepare("CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, abbr TEXT, desc TEXT)")
        create.run()
        ...
        const insert = db.prepare('INSERT INTO test(name, abbr, desc) VALUES (?, ?, ?)')
